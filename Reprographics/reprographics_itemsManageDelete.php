...
if (!isActionAccessible($guid, $connection2, '/modules/Reprographics/reprographics_itemsManage.php')) {
    ...
    if (empty($itemID) || !$itemGateway->exists($itemID)) {
        $page->addError(__('No Item Selected.'));
    } else {
        $form = DeleteForm::createForm($gibbon->session->get('absoluteURL') . '/modules/' . $gibbon->session->get('module') . "/reprographics_itemManageDeleteProcess.php");
