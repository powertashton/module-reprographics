...
if (!isActionAccessible($guid, $connection2, '/modules/Reprographics/reprographics_categoryManage.php')) {
    ...
    if (empty($subCategoryID) || !$subCategoryGateway->exists($subCategoryID)) {
        $page->addError(__('No SubCategory Selected.'));
    } else {
        $form = DeleteForm::createForm($gibbon->session->get('absoluteURL') . '/modules/' . $gibbon->session->get('module') . "/reprographics_subCategoryManageDeleteProcess.php");
